...
function parseBinaryPlist(buffer) {
  ...
  function readLengthAndOffset(objectOffset, info) {
    if (info < 15) {
      return { length: info, dataOffset: objectOffset + 1 };
    }
    const intByteSize = 1 << (bytes[objectOffset + 1] & 0x0f);
    return {
      length: readSizedInt(objectOffset + 2, intByteSize),
      dataOffset: objectOffset + 2 + intByteSize
    };
  }
  ...
  const parseObject = (objectOffset) => {
    ...
    if (type === 0x2) {
      const { length, dataOffset } = readLengthAndOffset(objectOffset, info);
      return readSizedInt(dataOffset, length);
    }
    if (type === 0x3) {
      const { length, dataOffset } = readLengthAndOffset(objectOffset, info);
      if (length === 4) return bytes.readFloatBE(dataOffset);
      if (length === 8) return bytes.readDoubleBE(dataOffset);
    ...
    if (type === 0x8) {
      if (info < 15) {
        return { UID: readSizedInt(objectOffset + 1, info + 1) };
      }
      const intByteSize = 1 << (bytes[objectOffset + 1] & 0x0f);
      return { UID: readSizedInt(objectOffset + 2, intByteSize) };
    }
    if (type === 0xa) {
      const { length, dataOffset } = readLengthAndOffset(objectOffset, info);
      const values = [];
      for (let index = 0; index < length; index += 1) {
        const ref = readSizedInt(dataOffset + index * objectRefSize, objectRefSize);
        values.push(ref);
      }
    ...
    if (type === 0xd) {
      ...
      const entries = {};
      for (let index = 0; index < length; index += 1) {
        const keyRef = readSizedInt(dataOffset + index * objectRefSize * 2, objectRefSize);
        const valueRef = readSizedInt(dataOffset + index * objectRefSize * 2 + objectRefSize, objectRefSize);
